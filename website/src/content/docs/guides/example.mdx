...
Once logged in, you can navigate to your organizations on the panel on the lefthand side of the page.

<div style="display:flex; justify-content:center">
![Organization button highlighted](../../../assets/getting-started-assets/Organizations-button-Flowcore-io.png) 
</div>

You can manage your organization by:
...
To create a new organization, simply navigate to the top right corner of the **Organizations** tab and press the **New Organization** button.

## Creating a Data Core

<YouTube id="https://www.youtube.com/watch?v=IdPruN6Zyic" />

Here we will provide a detailed overview of data cores, explaining how they function and guiding you through the process of creating them. Additionally, we'll explore how to load data into these data cores using webhooks.
...
To create a data core, navigate to your organization and press the **Create Datacore** button, located on the righthand side. Give your data core a name, relevant to the data you will be assigning to it, and give a brief description of what it is. 
...
Within the data core, you can organize your data further by creating groups and subgroups. This is done by creating **Flowtypes** from within the data core. To simplify the understandings flowtypes, you can imagine that the aggrigator is a drawer and the event types are little storage boxes within the drawer. This gives you the ability to roughly organize your data without having to be too thorough. 
...
Next, we explore how to send data into a data core through ingestion channels. This can be found in the panel on the lefthand side. 
...
Here you can choose which organization, data core, flowtype and event type to use. This will generate a webhook, which is a URL you can call, for you. However, to complete the webhook, you will need to generate an API key for authentication.
...
The benefit to using these API keys is the ease to delete them at will, in case you should choose to revoke access to your data core. 
...
Open Insomnia and create a new HTTP request and set it to POST. There you can paste the webhook you generated. We will be sticking with JSON, as that is what the webhooks support. This is where the actual data is used. The data can now be sent into your data core. You can see this in your data core on **[Flowcore](https://flowcore.io/)**. 

<div style="display:flex; justify-content:center">
![Insomnia](../../../assets/getting-started-assets/insomnia-data.png) 
</div>

The whole structure of the data is defined by the data you send in, so it is completely schema-less. 

To send data to a different event type, you can either generate a new webhook, using the same API key, or you can manually change it in the webhook URL. 

## Creating a Read Model (with PlanetScale)

<YouTube id="https://www.youtube.com/watch?v=xB4l5pvIkTs" />

Here we will explore the concept of read models in Flowcore and learn how to connect them to external sources. Read models are vital for retrieving data from Flowcore and transferring it to your own systems, such as databases or storage solutions. This guide provides an introduction to read models and walks you through connecting a read model to an external source, specifically using PlanetScale to set up a database.

:::note
You will need an account on **[PlanetScale](https://planetscale.com/)** to create a database there.
:::
...
<div style="display:flex; justify-content:center">
![Create database](../../../assets/getting-started-assets/create-new-database_planetscale.png) 
</div>
...
To establish the connection to the external database, we must provide the necessary configuration details. To connect to the PlanetScale database, enter the database name, username, password, and host information. 
...
In this case, the database uses SSL for secure connections. Once these details are entered, the read model is created and ready for data transfer.
...
In your selected database, click the **Connect** button on the top right side. 
...
You specify the organization and data core you want to use for this scenario. The data cores you use can be part of your organization, or you can subscribe to public data cores from other organizations. 
...
You can see the status of deployment my hovering over the little cloud icon on the bottom left corner of your adapter node. This can take a minute. 
...
<div style="display:flex; justify-content:center">
![GitHub search](../../../assets/getting-started-assets/github-search-bar.png) 
</div>

To do so, go to GitHub and search for **Flowcore.io**.

<div style="display:flex; justify-content:center">
![GitHub repository examples](../../../assets/getting-started-assets/example-repositories.png) 
</div>

There you can search for templates (for typescript and python), which you can use to get started. You can of course also make and use your own, but we will be using the examples we made, for this guide. So go ahead and click the typescript template. 
...
Once you've made your own repository, following the [README file](https://github.com/flowcore-io/nodejs-typescript-transformer-example/blob/main/README.md), open it and press **.** on your keyboard to open the live editor in GitHub. 

( The example provided here demonstrates calculating the difference between the temperature data from the data core and the optimal heating temperature within a building. The transformed data will include this calculated value. This example is just a starting point, and you can modify it to fit your specific needs.

Set up a constant for the optimal heating temperature, e.g., 22 degrees.
Extract the temperature from the input data, convert it to a number, and calculate the difference.
Assign the calculated value to a field within the transformed data.

To manage the transformation code, we use GitHub, where you create your repository for full control over your transformation and endpoints. The process is documented in detail within the [README file](https://github.com/flowcore-io/nodejs-typescript-transformer-example/blob/main/README.md) for setting up your own project.

After creating and configuring your GitHub repository, you'll generate a release (e.g., Version 1.0.0) that includes your transformation code. This release serves as the point of integration with the Flowcore platform.

Copy the link to the release zip file from GitHub.
In Flowcore, set up an adapter and provide the link to the zip file.
Configure the transformation execution environment, specifying the language (TypeScript, in this case).
Complete the adapter settings and connect it to the desired read model.
