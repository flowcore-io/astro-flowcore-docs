...
-   **Flow Type**: Similar to an aggregate in traditional event sourcing, but simpler - it's just a logical grouping for related events e.g. todo items
-   **Event Type**: This is where Flowcore differs significantly from classic event sourcing:
    -   In traditional event sourcing: You have **one stream per aggregate instance** (e.g., one stream per `TodoItem`, like `TodoItem-001`, `TodoItem-002`, etc. with event types like `TodoItemCreated`, `TodoItemUpdated`)
    -   In Flowcore: You have **one stream per event type**. Instead of `TodoItem` instances, you have `TodoItemCreated`, `TodoItemUpdated`, `TodoItemDeleted` as separate streams
    -   We also encourage different naming conventions for Event Types, like `todo-item.created`, `todo-item.updated`, `todo-item.deleted`
