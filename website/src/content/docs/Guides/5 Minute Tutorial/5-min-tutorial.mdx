...
No Kafka. No CDC connectors. No current-state rebuild/rehydration to validate user actions.
...
-   [Flowcore Platform account](https://flowcore.io) to get your Tenant name
...
-   **Flow Type**: Similar to a domain or an aggregate in traditional event sourcing, but simpler - it's just a title for grouping related immutable event logs e.g. `TodoItem`
...
pathways.handle("todo-items/todo-item.created.v0", async ({ payload }) => {
  ...
  `;
    if (result.length === 0) {
        ...
        console.log("Todo Item ALREADY EXISTS (skipping)", payload);
    }
});
...
Create `write-demo.ts` to send test events. In your applications you should do business logic checks before sending these events:
...
await pathways.write("todo-items/todo-item.renamed.v0", {
    data: { id: milkId, newTitle: "Buy oat milk" },
});
...
await pathways.write("todo-items/todo-item.renamed.v0", {
    data: { id: breadId, newTitle: "Buy wholemeal bread" },
});
...
```bash
flowcore scenario local -f flowcore.local.yaml -s first -e http://localhost:3000/api/transformer -H 'X-Secret: 1234'
```
