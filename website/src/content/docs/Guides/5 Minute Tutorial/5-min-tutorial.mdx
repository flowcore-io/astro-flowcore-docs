...
To validate user actions run SQL checks against real-time read models, get your answer in milliseconds, and emit an immutable event.
...
version: 1
tenant: ez # <-- Replace with name of your tenant
dataCore:
    name: todo-app
    flowTypes:
        todo-items:
...
Create `pathways.ts` to configure Flowcore:
