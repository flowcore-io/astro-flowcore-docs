...
When you receive the event back from Flowcore you materialize and store it in any way you want.

No Kafka. No Domain Driven Design. No current-state rebuild/rehydration to validate user actions.

Flowcore is responsible for storing the events that you emit, in immutable event logs, and fanning them out to subscribed services, in high-throughput critical situations,
in single digit milliseconds or, in lower throughput less critical situations, in single digit seconds.
...
import { pathways, postgresUrl } from "./pathways";

const sql = postgres(postgresUrl);

pathways.handle("todo-items/todo-item.created.v0", async (event) => {
    console.log("📥 Todo created:", event.payload.title);
    await sql`
      INSERT INTO todo (id, title, description, done)
      VALUES (${event.payload.id}, ${event.payload.title}, ${
...
pathways.handle("todo-items/todo-item.updated.v0", async (event) => {
    console.log("📝 Todo updated:", event.payload.title);
    await sql`
      UPDATE todo 
      SET title = ${event.payload.title}, description = ${
        event.payload.description || null
...
pathways.handle("todo-items/todo-item.deleted.v0", async (event) => {
    console.log("🗑️ Todo deleted:", event.payload.id);
    await sql`DELETE FROM todo WHERE id = ${event.payload.id}`;
...
1. **Event Written**: `pathways.write()` sends an event to Flowcore
2. **Event Stored**: Flowcore stores the event
