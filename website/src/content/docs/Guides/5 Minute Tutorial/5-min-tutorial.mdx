...
-   **Flow Type**: Similar to an aggregate in traditional event sourcing, but simpler - it's just a logical grouping for related events e.g. `Order`
-   **Event Type**: This is where Flowcore differs significantly from classic event sourcing:
    -   In traditional event sourcing: **You have one stream per aggregate instance** (e.g., one stream per Order, like Order-0001, Order-0002, etc. with event types like `OrderCreated`, `OrderUpdated`)
    -   In Flowcore: You have **one stream per event type**. Instead of `Order` instances, you have `OrderCreated`, `OrderUpdated`, `OrderArchived` as separate streams
