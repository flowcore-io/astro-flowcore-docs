...
A data pathway connects selected Flowcore event types to application handlers. In production, managed pathways deliver events to an authenticated HTTP endpoint. Applications define contracts and handlers with `@flowcore/pathways`.

- **Example**: An `orders-service` pathway delivers `order/placed.0` events to the service endpoint.
...
- **Synchronization**: Data Pathway handlers keep read models in sync from immutable events
