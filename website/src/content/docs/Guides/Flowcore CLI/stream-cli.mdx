...
```sh
flowcore stream "https://flowcore.io/<tenant>/<data-core>/*"
```

or to stream all events from a Flow Type, you can use the `*` wildcard to stream all events from the Flow Type.

```sh
flowcore stream "https://flowcore.io/<tenant>/<data-core>/<flow-type>/*"
```
...
<Card title="JSON" icon="document">
    ...
    ```sh
    flowcore stream "stream url" -s 1h --json > output.json
    ```
</Card>

## Local HTTP delivery

Use `stream http` to send selected events to a local endpoint while developing:

```sh
flowcore stream http \
  "https://flowcore.io/<tenant>/<data-core>/<flow-type>/<event-type>.stream" \
  --start now \
  --destination http://localhost:3000/api/flowcore \
  --header "X-Webhook-Secret: local-secret"
```

This process runs only while the command is active. Production consumers should use a managed data pathway.
...
```sh
flowcore stream "stream url" -s 1h --payload
```
...
```sh
flowcore stream "stream url" -s now
```
...
```sh
flowcore stream "stream url" -m 100
```
...
```sh
flowcore stream "stream url" --json
```
...
```sh
flowcore stream http "stream url" -d http://localhost:3000/api/transform
```
...
```sh
flowcore stream http "stream url" -d http://localhost:3000/api/transform -H "Authorization: Bearer token"
```
...
```sh
flowcore stream "stream url" --no-live
```
...
```sh
flowcore stream "stream url" -s 1h --no-live
```
...
```sh
flowcore stream "stream url" -s 2022-01-01T00:00:00Z -e 2022-01-02T00:00:00Z
```
