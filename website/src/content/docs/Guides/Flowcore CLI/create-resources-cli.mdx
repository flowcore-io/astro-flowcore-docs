...
By default, transformers are created with a default start date of `now()`. You can set the default start date of the transformer by setting the `defaultStartDateTime` field to a number in the manifest file.
