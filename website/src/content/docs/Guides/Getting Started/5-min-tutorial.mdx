...
sidebar:
    order: 1
description: A 5 minute explanation and tutorial with Flowcore
---

Flowcore shatters the idea that event sourcing must be complex: instead of juggling Kafka topics, CQRS layers and Domain Driven Design,
we record every action as an immutable event, we use proxy endpoints for real-time fan-out to keep downstream services in-sync,
...
1. You make a simple manifest (YAML) file to create your event architecture, and use the CLI to apply it on the Flowcore Platform
2. To validate a user action before you emit an event to Flowcore you do SQL business logic checks against your database
3. When an event is emitted to Flowcore it is stored in the designated Event Type (event stream) and is fanned out to all subscribers
4. Your Application subscribes to the event stream and receive the event in real-time. Which is when you update your read model.
5. You can replay the event stream to rebuild or create new read models on demand.
